...
def move_plan_priority_calc(mecs:MEC_servers, device:Device, plan_index, time, f_time, continue_distance):
    ...
    mec_num = len(mecs)
    mec = None
    mec_set = collections.namedtuple("mec_set", ("ID", "way"))  #MEC名と距離

    # 各MECサーバとの距離を計算
    for m in range(mec_num):
        print(device.plan_index, f_time, len(device.plan))
        #先読み時間(ftime)後にデバイスがいる位置ときの距離を計算
       ...
       # 探索距離内(継続割り当て距離内)にあるサーバを調べ、見つかったら配列に追加
        if disntace <= continue_distance:
           #found_judge = True
           if mec is None:
               mec = [mec_set(mecs[m].name, disntace)]         #配列の1つ目の要素に追加する
           else:
               mec.append(mec_set(mecs[m].name, disntace))     #配列の2つ目以降の要素に追加する
    ...
    if mec is not None:
            ...
            array_num = len(tmp_mecs)  #配列のサイズを取得
            ...
            for i in range(array_num):
                if sorted_mecs is None:
                    sorted_mecs = [search_mec(mecs, tmp_mecs[i].ID)]      #配列にMECの名前からMECのインスタンスを取得して代入
                    sorted_mecs[0]._sorted_flag = True                    #sorted_flag = Trueに変更
                else:
                    sorted_mecs.append(search_mec(mecs, tmp_mecs[i].ID))  #追加
                    sorted_mecs[i]._sorted_flag = True                    #変更
    ...
    if check_between_time(device, time) == True:
        ...
        for m in range(sort_finish_index):
            ...
            if (mecs[m].check_resource(device.use_resource) == True) and (mecs[m].resource > 0) and (plan_index < len(device.plan)):
                ...
                #MECの実行可能なAP環境で実行可能かどうか判定する
                for t in range(ap_num):
                    if device.app_ver == mecs[m].app_ver[t]:
                        run_flag = True
                ...
                if run_flag == True:   
                    ...
                    #割り当てに成功した場合
                    if device_flag == True:
                        return True, allocated_mec_id     #割り当てたMECの名前を返す
...
def mode_adjustment(mecs:MEC_servers, mec_index, device: Device, time):
    ...
    current_id = mecs[mec_index].name                   #MECの名前を保存
    print("割り振るMEC_ID", current_id, "前に割り振ったMEC_ID", device.mec_name)
    mec_index = search_mec_index(mecs, current_id)      #MECのインデックスを取得

    #今回割り当てるMECがすでに割り当てたMECと同じ時
    if (current_id == device.mec_name):
        ...
        #mecs[mec_index].custom_resource_adjustment(device, time) keepのため必要ない
    ...
    # 切替&切替先を見つけている時
        ...
        # リソース調整
        mecs[previous_index].custom_resource_adjustment(device, time)   #以前に割り当てていたリソースを調整(増やす)
        ...
        print("ADD")
        hop_calc(device, mecs, mecs[mec_index], previous_mec_name, time)  #ホップ数の計算
        ...
        mecs[mec_index].add_having_device(time)
        mecs[mec_index].custom_resource_adjustment(device, time)         #MECサーバのリソース量を調整する(減らす)
        mecs[mec_index].save_resource(time)
    ...
    # 新規割り当て
        ...
        #device.add_hop_countが1箇所ない 必要ないの?
